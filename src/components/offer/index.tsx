...
export function Offer({ item }: { item: ItemDto }) {
	...
	return (
		<li className='flex flex-col gap-3 text-slate-900 bg-white p-5 rounded-lg'>
			<div className='flex justify-between'>
				<div className='flex gap-3'>
					<div
						className='bg-slate-600 rounded-lg w-7 max-w-xs aspect-[1/1]'
						style={{ minWidth: '50px' }}>
						<img
					...
					<div>
						...
						<span className='text-slate-600'>
							{item.teleworking?.value} · {item.salaryDescription}{' '}
							· {item.city}
						</span>
					</div>
				</div>
			...
			<div className='flex gap-3'>
				<span className='rounded-full px-5 py-2 bg-slate-200 text-xs'>
